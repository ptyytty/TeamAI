...
def main(selected_model, epoch_count):
    ...
    for model_name, model in models_to_train.items():
        ...
        val_dataset = CustomDataset(image_dir=PREDICT_DIR, csv_file=TEST_CSV_PATH,transform=predict_transform,is_training=True)
        val_loader = DataLoader(val_dataset,batch_size=32,shuffle=False,pin_memory=True,num_workers=4)

        # 모델 학습 및 가장 좋은 가중치 저장
        best_f1, best_model_wts, total_time, train_f1_scores, val_f1_scores = train_model(
            model, 
